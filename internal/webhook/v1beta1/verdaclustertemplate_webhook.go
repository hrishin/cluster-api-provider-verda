...
func SetupVerdaClusterTemplateWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr, &infrav1.VerdaClusterTemplate{}).
		...
		WithDefaulter(&VerdaClusterTemplateCustomDefaulter{}).
		Complete()
}
...
func (v *VerdaClusterTemplateCustomValidator) ValidateUpdate(ctx context.Context, oldObj, newObj *infrav1.VerdaClusterTemplate) (admission.Warnings, error) {
	...
	errs := validateClusterSpec(&newObj.Spec.Template.Spec, field.NewPath("spec", "template", "spec"))
	if !topology.IsDryRunRequest(req, newObj) && !reflect.DeepEqual(oldObj.Spec.Template.Spec, newObj.Spec.Template.Spec) {
		errs = append(errs, field.Forbidden(field.NewPath("spec", "template", "spec"), "VerdaClusterTemplate spec is immutable; create a new template to change clusters"))
	}
