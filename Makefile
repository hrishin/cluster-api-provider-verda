...
# E2E tests create real clusters on Verda through the management cluster in
# the current kubeconfig context (provider CRDs installed and a manager
# running, e.g. "make install && make run"). They are skipped unless the
# E2E_VERDA_* variables and VERDA_CLIENT_ID/VERDA_CLIENT_SECRET are set.
.PHONY: test-e2e
test-e2e: ## Run the e2e tests against Verda. Billable; needs E2E_VERDA_LOCATION, E2E_VERDA_OS_VOLUME_ID, E2E_VERDA_SSH_KEY_ID.
...
.PHONY: run
run: manifests generate fmt vet ## Run a controller from your host (webhooks disabled: the API server cannot reach them).
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name cluster-api-provider-verda-builder
	$(CONTAINER_TOOL) buildx use cluster-api-provider-verda-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm cluster-api-provider-verda-builder
	rm Dockerfile.cross
...
release-manifests: manifests generate kustomize ## Generate the clusterctl release assets in out/.
	...
	cp templates/cluster-template*.yaml out/
